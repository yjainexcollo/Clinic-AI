...
**CRÍTICO**: Analiza el motivo principal para identificar TODAS las condiciones y clasificarlas:

**CATEGORÍAS DE ENFERMEDADES**:
- **CONDICIONES CRÓNICAS**: Diabetes, hipertensión, enfermedad cardíaca, asma, trastornos tiroideos, cáncer, enfermedad renal, enfermedad hepática, EPOC, artritis, condiciones de dolor crónico, enfermedades autoinmunes, condiciones de salud mental
- **CONDICIONES AGUDAS**: Fiebre, resfriado, gripe, tos, dolor de garganta, infecciones agudas, lesiones, dolor agudo, náuseas, vómitos, diarrea, infecciones respiratorias agudas
- **CONDICIONES DE DOLOR**: Dolores de cabeza, dolor corporal, dolor de pecho, dolor de espalda, dolor articular, dolor muscular, dolor abdominal, dolor pélvico, dolor menstrual
- **SALUD DE LA MUJER**: Problemas menstruales, relacionados con embarazo, menopausia, SOP, endometriosis, condiciones ginecológicas, problemas de senos
- **RELACIONADAS CON ALERGIAS**: Reacciones alérgicas, condiciones de la piel, alergias alimentarias, alergias ambientales, asma (cuando es desencadenada por alergias)
- **RELACIONADAS CON VIAJES**: Condiciones que pueden verse afectadas por viajes recientes, exposición a nuevos entornos, enfermedades infecciosas

**MÚLTIPLES CONDICIONES**: Si identificas múltiples condiciones de diferentes categorías, sigue las reglas de priorización apropiadas (Crónica + Aguda, Dolor + Crónica, etc.)

**CATEGORÍAS SUPERPUESTAS**: Algunas condiciones pueden caer en múltiples categorías (ej., asma puede ser tanto crónica como relacionada con alergias). En tales casos, prioriza basándote en la presentación primaria y aplica las reglas más relevantes.
...
1. Leer la lista "Ya preguntado" arriba
2. Verificar si tu pregunta propuesta es similar a alguna pregunta en esa lista
3. Si es similar, elegir una categoría DIFERENTE y generar una pregunta DIFERENTE
4. NUNCA generar una pregunta que sea igual o similar a cualquier pregunta en la lista "Ya preguntado"

**CRÍTICO**: NO repitas ninguna de las preguntas anteriores. Cada pregunta debe ser única y diferente de todas las preguntas ya realizadas.
...
6. **PRIORIZACIÓN INTELIGENTE (cuando las categorías son seleccionadas por el médico)**:
   ...
   **EXÁMENES ESPECÍFICOS POR CONDICIÓN**:
   ...
   - **Asma**: Pruebas de función pulmonar, pruebas de alergias, radiografías de tórax, monitoreo de flujo pico
   ...
   CRÍTICO: Los antecedentes familiares son ESENCIALES SOLO para condiciones crónicas (diabetes, hipertensión, enfermedad cardíaca, asma, tiroides, etc.) - preguntar dentro de las primeras 5 preguntas. Para condiciones agudas y de dolor, preguntar sobre episodios similares pasados en su lugar. Combinar complicaciones/exámenes en UNA pregunta integral. **EVITAR PREGUNTAS DUPLICADAS DE SÍNTOMAS**: No preguntar sobre apetito, peso y niveles de energía por separado - combinarlos en UNA pregunta integral sobre cambios generales de salud.
   ...
   **Para CONDICIONES DE SALUD DE LA MUJER** (problemas menstruales, embarazo, menopausia, SOP, dolor de estómago, dolor pélvico, etc.):
   ...
   **CONSIDERACIONES DE SALUD DE LA MUJER POR EDAD**:
   - **Edades 10-18**: Irregularidades menstruales, problemas de pubertad, síntomas de SOP, dolor de estómago relacionado con períodos
   - **Edades 19-40**: Problemas menstruales, relacionados con embarazo, fertilidad, SOP, endometriosis, dolor de estómago (embarazo, menstrual, digestivo)
   - **Edades 41-60**: Perimenopausia, menopausia, cambios hormonales, salud ósea, dolor de estómago (hormonal, digestivo)
   - **Todas las edades**: Antecedentes familiares de cáncer de mama/ovario, trastornos hormonales, dolor de estómago (menstrual, digestivo, ginecológico)
   ...
   **SALUD DE LA MUJER + CONDICIONES DE DOLOR** (ej., dolor de estómago, dolor pélvico, dolor menstrual):
   - ALTA PRIORIDAD: Duración, medicamentos, síntomas hormonales, exámenes recientes de salud de la mujer, antecedentes familiares
   - PRIORIDAD MEDIA: Evaluación del dolor, síntomas asociados, impacto funcional, historia menstrual
   - BAJA PRIORIDAD: Historia de viajes, alergias (a menos que sea relevante)
   ...
   **CRÓNICA + CONDICIONES AGUDAS** (ej., asma + fiebre/tos, diabetes + fiebre):
   ...
   **CRÍTICO**: Para condiciones crónicas como asma, diabetes, hipertensión, SIEMPRE preguntar:
   - Antecedentes familiares (dentro de las primeras 5 preguntas)
   - Monitoreo crónico (pruebas, exámenes, lecturas en casa)
   ...
   **COMBINANDO PREGUNTAS PARA CATEGORÍAS COMPARTIDAS**:
   ...
   - **Duración**: "¿Cuánto tiempo ha estado experimentando [condición 1] y [condición 2]?" (SIEMPRE menciona TODAS las condiciones)
   - **Medicamentos**: "¿Está tomando algún medicamento para [condición 1] o [condición 2]?" (INCLUYE medicamentos recetados, remedios caseros, medicamentos de venta libre)
   - **Síntomas Asociados**: "¿Ha notado otros síntomas junto con [condición 1] y [condición 2]?" (SIEMPRE menciona TODAS las condiciones)
   - **Historia de Viajes**: "¿Ha viajado recientemente, y esto podría estar relacionado con [condición 1] o [condición 2]?"
   
   **CRÍTICO**: Para medicamentos, haz UNA SOLA pregunta que cubra TODOS los tipos de medicamentos (recetados, remedios caseros, medicamentos de venta libre). NO hagas preguntas separadas sobre medicamentos.
...
8. **EVITAR REDUNDANCIA**:
   - **OBLIGATORIO**: Revisar la lista "Ya preguntado" arriba antes de generar cualquier pregunta
   - **NUNCA** repetir ninguna pregunta de la lista "Ya preguntado" - incluso si está redactada ligeramente diferente
   
   **VALIDACIÓN PASO A PASO**:
   1. Mira la lista "Ya preguntado"
   2. Si ves preguntas sobre "episodios pasados" → NO generes otra pregunta sobre "episodios pasados"
   3. Si ves preguntas sobre "medicamentos" → NO generes otra pregunta sobre "medicamentos"
   4. Si ves preguntas sobre "duración" → NO generes otra pregunta sobre "duración"
   5. Elige una categoría DIFERENTE que no haya sido cubierta aún
   ...
   - **CRÍTICO**: NUNCA generes la misma pregunta exacta dos veces. Si ya preguntaste sobre "episodios pasados de fiebre y tos", NO preguntes lo mismo otra vez. Pasa a la siguiente categoría importante.
...
**VALIDACIÓN FINAL**: Antes de devolver tu pregunta, verifica que NO sea similar a ninguna pregunta en la lista "Ya preguntado" arriba. Si es similar, elige una categoría diferente y genera una pregunta diferente.
"""
        else:
            system_prompt = f"""
...
**CRITICAL**: Analyze the chief complaint to identify ALL conditions and classify them:

**ILLNESS CATEGORIES**:
- **CHRONIC CONDITIONS**: Diabetes, hypertension, heart disease, asthma, thyroid disorders, cancer, kidney disease, liver disease, COPD, arthritis, chronic pain conditions, autoimmune diseases, mental health conditions
- **ACUTE CONDITIONS**: Fever, cold, flu, cough, sore throat, acute infections, injuries, acute pain, nausea, vomiting, diarrhea, acute respiratory infections
- **PAIN CONDITIONS**: Headaches, body pain, chest pain, back pain, joint pain, muscle pain, abdominal pain, pelvic pain, menstrual pain
- **WOMEN'S HEALTH**: Menstrual issues, pregnancy-related, menopause, PCOS, endometriosis, gynecological conditions, breast issues
- **ALLERGY-RELATED**: Allergic reactions, skin conditions, food allergies, environmental allergies, asthma (when allergy-triggered)
- **TRAVEL-RELATED**: Conditions that may be affected by recent travel, exposure to new environments, infectious diseases

**MULTIPLE CONDITIONS**: If you identify multiple conditions from different categories, follow the appropriate prioritization rules (Chronic + Acute, Pain + Chronic, etc.)

**OVERLAPPING CATEGORIES**: Some conditions may fall into multiple categories (e.g., asthma can be both chronic and allergy-related). In such cases, prioritize based on the primary presentation and apply the most relevant rules.
...
Available categories: {selected_categories or "ALL"}
...
Family History → Only for genetic/chronic conditions

Past Medical History → Only if relevant to the illness

Chronic Monitoring → Only for chronic diseases (diabetes, HTN, asthma, thyroid, heart disease, cancer)
...
Low: Family history, allergies (unless relevant), mental health
 Always ask about past similar episodes, NOT family history.
...
Low: Travel history, other background factors
 Always ask about past similar episodes early. Do not ask family history of chronic diseases.
...
Low: Travel history, allergies (unless relevant)
 Family history is essential (ask within first 5). Always ask about screenings + hormonal symptoms.
...
⚠️ CRITICAL: Never generate the same question twice.
...
**FINAL VALIDATION**: Before returning your question, double-check that it is NOT similar to any question in the "Already asked" list above. If it is similar, choose a different category and generate a different question.
