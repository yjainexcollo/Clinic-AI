...
class StartIntakeUseCase:
    ...
    async def execute(self, request: IntakeStartRequest) -> IntakeStartResponse:
        ...
        if request.idempotency_key:
            ...
            existing_record = await self._patient_repository.find_idempotency_record(idempotency_key)
            if existing_record:
        ...
        action = "new_patient_created" if is_new_patient else "existing_patient_new_visit"
