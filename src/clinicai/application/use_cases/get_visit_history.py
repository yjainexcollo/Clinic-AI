...
class GetVisitHistoryUseCase:
    ...
    async def execute(self, request: VisitHistoryRequest) -> VisitHistoryResponse:
        ...
        visits = await self._patient_repository.get_patient_visits(
            patient_id, limit=request.limit, offset=request.offse
        )
            ...
            visit_summaries.append(
                VisitSummaryDTO(
                    ...
                    completed_at=(
                        ...
                        if visit.intake_snapshot and visit.intake_snapshot.completed_a
                        else None
                    ),
                    total_questions=(
                        visit.intake_snapshot.total_questions
                        if visit.intake_snapsho
                        else 0
                    ),
