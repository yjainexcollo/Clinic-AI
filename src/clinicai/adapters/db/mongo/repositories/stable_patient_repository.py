...
class MongoStablePatientRepository(StablePatientRepository):
    ...
    async def _domain_to_mongo_patient(
        self, patient: StablePatient
    ) -> StablePatientMongo:
    ...
    async def _mongo_to_domain_patient(
        self, patient_mongo: StablePatientMongo
    ) -> StablePatient:
    ...
    async def _domain_to_mongo_visit(self, visit: StableVisit) -> StableVisitMongo:
        ...
        return StableVisitMongo(
            ...
            notes=visit.notes,
            idempotency_key=(
                visit.idempotency_key.value if visit.idempotency_key else None
            ),
        )

    async def _mongo_to_domain_visit(
        self, visit_mongo: StableVisitMongo
    ) -> StableVisit:
        ...
        return StableVisit(
            ...
            idempotency_key=(
                IdempotencyKey.from_string(visit_mongo.idempotency_key)
                if visit_mongo.idempotency_key
                else None
            ),
        )
